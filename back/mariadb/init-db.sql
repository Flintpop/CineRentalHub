...
BEGIN
    IF NEW.cart_type = 'rental' AND NEW.rental_duration <= 0 THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'La durée de location doit être supérieure à 0.';
    END IF;
END; //
DELIMITER ;
...
BEGIN
    IF EXISTS (
        ...
        SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = 'Le même utilisateur ne peut pas louer le même film sur des périodes qui se chevauchent.';
    END IF;
END; //
DELIMITER ;
...
DELIMITER //
CREATE PROCEDURE add_movie(IN title VARCHAR(255), IN release_date DATE, IN description TEXT, IN daily_rental_price DECIMAL(10, 2), IN purchase_price DECIMAL(10, 2), IN link VARCHAR(2000))
BEGIN
...
DELIMITER //
CREATE PROCEDURE update_movie(IN movie_id INT, IN title VARCHAR(255), IN release_date DATE, IN description TEXT, IN daily_rental_price DECIMAL(10, 2), IN purchase_price DECIMAL(10, 2), IN link VARCHAR(2000))
BEGIN
    ...
    ELSE
        UPDATE movies
        SET title = title, release_date = release_date, description = description, daily_rental_price = daily_rental_price, purchase_price = purchase_price, link = link
        WHERE id = movie_id;
    END IF;
END //
DELIMITER ;
...
DELIMITER //
CREATE PROCEDURE create_user(IN last_name VARCHAR(255), IN first_name VARCHAR(255), IN new_email VARCHAR(255), IN password CHAR(255), IN role ENUM('user', 'admin'))
BEGIN
    ...
    ELSE
        INSERT INTO users (last_name, first_name, email, password, role, activated) VALUES (last_name, first_name, new_email, password, role, TRUE);
    END IF;
END //
DELIMITER ;
...
DELIMITER //
CREATE PROCEDURE update_user(IN user_id INT, IN new_last_name VARCHAR(255), IN new_first_name VARCHAR(255), IN new_email VARCHAR(255), IN new_activated BOOLEAN, IN new_role ENUM('user', 'admin'))
BEGIN
    ...
    ELSE
        UPDATE users SET last_name = new_last_name, first_name = new_first_name, email = new_email, activated=new_activated, role = new_role WHERE id = user_id;
    END IF;
END //
DELIMITER ;
...
DELIMITER //
CREATE PROCEDURE add_to_cart(IN user_id INT, IN movie_id INT, IN cart_type ENUM('purchase', 'rental'), IN rental_duration INT)
BEGIN
    IF NOT EXISTS (SELECT id FROM users WHERE id = user_id) THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'L\'utilisateur n\'existe pas.';
...
BEGIN
    ...
    ELSEIF NOT EXISTS (SELECT id FROM movies WHERE id = movie_id) THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Le film n\'existe pas.';
    ELSE
        INSERT INTO rentals (user_id, movie_id, rental_date, return_date) VALUES (user_id, movie_id, rental_date, return_date);
    END IF;
END //
DELIMITER ;
...
BEGIN
    IF NOT EXISTS (SELECT id FROM users WHERE id = user_id) THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'L\'utilisateur n\'existe pas.';
    ELSEIF NOT EXISTS (SELECT id FROM movies WHERE id = movie_id) THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Le film n\'xiste pas.';
    ELSE
        INSERT INTO comments (movie_id, user_id, comment_text, comment_date) VALUES (movie_id, user_id, comment_text, comment_date);
    END IF;
END //
DELIMITER ;
